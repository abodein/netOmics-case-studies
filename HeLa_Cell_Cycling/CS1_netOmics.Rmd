...
We get raw data from [@aviner2015uncovering].

Multi-omics data is composed of:

- mRNA
- Translation Data (iBAQ and LFQ normalisation)
- Proteomics  (iBAQ and LFQ normalisation)

For Translation and Proteomics, we selected iBAQ normalisation.

```{r preprocessing_1}
all_data <- openxlsx::read.xlsx("data/pgen.1005554.s012.xlsx", startRow = 2)

# ID conversion from uniprot retriver
conv <- read_tsv("./data/uniprot_mapping.tab") %>% unique

type <- c("Translation.(LFQ)", "Translation.(iBAQ)", "Protein.(iBAQ)", 
          "Protein.(LFQ)", "mRNA")
DATA <- purrr::map(type, ~{all_data %>% 
        dplyr::select(Leading.Uniprot.IDs, contains(.x))}) %>%
    purrr::map(~ .x %>% as_tibble %>% 
                   column_to_rownames("Leading.Uniprot.IDs") %>% t 
               %>% as.data.frame(stringsAsFactor = FALSE))
names(DATA) <- type

DATA <- lapply(DATA, 
               function(x) `rownames<-`(x,rownames(x) %>% str_remove(".*\\.")))

mRNA <- DATA$mRNA %>% t %>% as.data.frame %>% na.omit() %>% 
    t %>% as.data.frame()

trans <- lapply(DATA$`Translation.(iBAQ)`, function(x) 
    as.character(x) %>% as.numeric) %>% 
    as.data.frame(row.names =  rownames(DATA$`Translation.(iBAQ)`)) %>%  
    t %>% as.data.frame %>% na.omit() %>% t %>% as.data.frame()

protein <- lapply(DATA$`Protein.(iBAQ)`, function(x) 
    as.character(x) %>% as.numeric) %>% 
    as.data.frame(row.names =  rownames(DATA$`Protein.(iBAQ)`)) %>%  
    t %>% as.data.frame %>% na.omit() %>% t %>% as.data.frame()

save(mRNA, trans, protein, file = "./result/norm_data.rda")
```

We display the coefficient of variation for each omic to adjust the fold-change
filtering step.
```{r preprocessing_1.2}
...
```
The filtering step, threshold:

- mRNA: log(2)
- trans: log(3)
- proteins: log(3)
```{r preprocessing_2}
mRNA.fc <- remove.fold.change(mRNA, log(2), 
                              hist.title = "mRNA FC log 2", plot = TRUE)

# Uniprot ID conversion to gene SYMBOL
mRNA.fc <- mRNA.fc %>% rownames_to_column("sample") %>% 
    gather(molecule, value, -sample) %>%
...
trans.fc <- remove.fold.change(trans, log(3),
                               hist.title = "trans FC log 3", plot = TRUE)
trans.fc <- trans.fc %>% rownames_to_column("sample") %>% 
    gather(molecule, value, -sample) %>%
    filter(molecule %in% conv$UNIPROT) %>% 
    dplyr::select(sample, molecule, value) %>% spread(molecule, value) %>%
    column_to_rownames("sample")

prot.fc <- remove.fold.change(protein, log(3), 
                              hist.title = "prot FC log 3", plot = TRUE)
prot.fc <- prot.fc %>% rownames_to_column("sample") %>% 
    gather(molecule, value, -sample) %>%
    filter(molecule %in% conv$UNIPROT) %>% 
    dplyr::select(sample, molecule, value) %>% 
...
# modeling
# median value
mRNA.fc %>% rownames_to_column("sample") %>% 
    mutate(group = sample %>% str_remove("_.*")) %>%
...
save(mRNA.fc, prot.fc, trans.fc, mRNA.modelled, trans.modelled, prot.modelled, file = "./result/modelled_data.rda")
```

# Modelling

Because there were only 3 timepoints (3 replicates), we tested 2 different way 
of modelling (median and LMMS).

```{r modelling_1}
time <- rownames(mRNA.fc) %>% str_remove("_.*") %>% 
    factor(levels = c("G1","S","G2/M")) %>% as.numeric

lmms.output <- lmms::lmmSpline(data = mRNA.fc, time = time,
                               sampleID = rownames(mRNA.fc), deri = FALSE,
                               basis = "cubic p-spline", 
                               numCores = 4, timePredict = 1:3, 
                               keepModels = TRUE)
...
save(mRNA.fc, prot.fc, trans.fc, mRNA.filter, trans.filter, prot.filter, 
     file = "./result/data_filtered.rda")
```

# Clustering

We tested 2 different clustering:

- timeOmics (with a block PLS).
- sign of variation (Up Down) as described in the paper

We tested the 2 clustering approaches on the 2 modeling approaches.

First with timeOmics:
```{r timeOmics_1}
data.mean <- list("mRNA" = mRNA.modelled, 
                  "trans" = trans.modelled, 
                  "prot"=prot.modelled)

cb.modelled <- do.call("cbind", data.mean)
colnames(cb.modelled) <- lapply(data.mean, colnames) %>% unlist

# 2 components gave the best clustering results (silhouette coefficient)
block.res <- block.pls(data.mean, ncomp = 2, 
                       indY = 1, scale = FALSE, mode = "canonical")
...
#2 filtered data
data.lmms <- list("mRNA" = mRNA.filter, 
                  "trans" = trans.filter, 
                  "prot"=prot.filter)
...
block.res <- block.pls(data.lmms, ncomp = 3, indY = 1, 
...
Second with the sign of variation:
```{r timeOmics_1-2}
######################################################
# cluster up/down
######################################################
...
sign_to_updown <- function(zz){
    ...
    for(i in 1:length(zz)){
      # print(z)
        ...
        r[i] <- data.frame("val" = z) %>% 
            mutate(conv = ifelse(val == 1, "Up", 
                                 ifelse(val == -1, "Down", "0"))) %>%
...
#1. modelled 
cluster.info.updown.mean <- imap_dfr(data.mean, 
                                     ~{up_down_cluster(.x) %>% 
                                             mutate(block = .y)})

# plot
cb.modelled %>% scale %>% as.data.frame() %>% rownames_to_column("time") %>% 
...
#2. lmms
cluster.info.updown.lmms <- imap_dfr(data.lmms, ~{up_down_cluster(.x) %>% 
        mutate(block = .y)})

# plot
png("./figure/CS1_clusters.png", height =  1500, width = 3000, res = 300)
cb.lmms %>% scale %>% as.data.frame() %>% rownames_to_column("time") %>% 
    gather(molecule, value, -time) %>%
  ...
  ggtitle("HeLa cell clycling data clusters, scale = TRUE") + xlab("Time") + 
    ylab("Scaled expression value")
dev.off()
...
tmp <- summary_cluster(cluster.info.updown.lmms) %>% 
    cbind(max = rowSums(., na.rm = TRUE)) %>% arrange(desc(max)) %>%
  dplyr::select(mRNA, trans, prot)
tmp[is.na(tmp)] <- 0
# in %
map2(tmp, colSums(tmp),~.x/.y) %>% as.data.frame() %>% 
    `row.names<-`(rownames(tmp))
save(cluster.info, file = "./result/luster_info.rda")
```
We then selected the best modelled/clustering approaches with the silhouette coefficient.

# Network(s) building

In this section we build the networks.

## Gene graph

For the Gene network, we used ARACNe [@aracne] algorithm (`minet` R package)
We build cluster specific networks and a global network (ALL).
...
GRN.cluster <- cluster.info %>% filter(block == "mRNA") %>% 
    split(.$cluster) %>%
    lapply(function(x) 
        dplyr::select(mRNA.fc, intersect(colnames(mRNA.fc), x$molecule)) %>%
               minet::build.mim() %>% aracne %>%
               graph_from_adjacency_matrix(mode = "undirected"))

par(mfrow = c(2,2))
imap(GRN.cluster, ~{
  plot(.x, main = .y,vertex.label = NA, vertex.size = 4, 
...
GRN.cluster[["all"]] = GRN
imap_dfr(GRN.cluster, ~get_stats_graph(.x) %>% as.data.frame %>% 
             mutate(cluster = .y)) %>% 
  mutate(cluster = factor(cluster, levels = c("Down_Up", "Up_Up", 
                                              "Down_Down", "Up_Down", "all"))) %>%
  arrange(cluster)

for(i in names(GRN.cluster)){
    vertex_attr(graph = GRN.cluster[[i]])$name <- 
        vertex_attr(graph = GRN.cluster[[i]])$name %>% str_remove("mRNA_")
    vertex_attr(graph = GRN.cluster[[i]])$type <- 
        rep("Gene", length(vertex_attr(graph = GRN.cluster[[i]])$name)) 
}

save(GRN.cluster, file = "./result/GRN.rda")
```

## Protein graph

We buid a PPI graph with BioGRID [@biogrid].
We filtered the database with the molecules from the experiment.

(This chunk is not evaluated because of the file size limit of github, only 
the result was uploaded).

```{r prot_graph_1, eval=FALSE}
# Proteins BIOGRID
biogrid <- read_tsv("data/BIOGRID-ALL-3.5.187.tab3.txt")
biogrid.filtered <- biogrid %>% 
    dplyr::select("Official Symbol Interactor A", "Official Symbol Interactor B") %>% 
    unique %>% set_names(c("A", "B"))
...
conv.biogrid <- AnnotationDbi::select(org.Hs.eg.db, keytype = "SYMBOL", 
                                      keys = c(biogrid.filtered$A, biogrid.filtered$B), 
                   columns = c("UNIPROT")) %>%
    unique %>% na.omit

biogrid.filtered <- biogrid.filtered %>% 
    left_join(conv.biogrid, by = c("A"="SYMBOL")) %>% dplyr::rename(A.UNIPROT = UNIPROT) %>%
...
```{r prot_graph_1-2, cache=TRUE}
load("data/BIOGRID.rda")
```

From biogrid we extracted all links between measured molecules and first degree neighbors (PPI_ego).
```{r prot_graph_2}
...
We build PPI subgraph for each cluster.
Based on the protein coding gene information (gene -> proteins),
we merged Gene networks and PPI networks (cluster-specific sub-networks and global network).
...
PPI_cluster[["all"]] <-  subgraph(PPI, v = intersect(
    lapply(prot.cluster, function(x) x$molecule) %>% unlist(), V(PPI)$name))
...
imap_dfr(PPI_cluster, ~{vertex_attr(.x) %>% as.data.frame() %>% mutate(cluster = .y)}) %>%
    ...
    mutate(cluster = factor(cluster, 
                            levels = c("Down_Up", "Up_Up", "Down_Down", "Up_Down", "all"))) %>%
    arrange(cluster) %>%
    column_to_rownames("cluster")

# stats
imap_dfr(PPI_cluster, ~get_stats_graph(.x) %>% 
             as.data.frame %>% mutate(cluster = .y)) %>% 
    mutate(cluster = factor(cluster, 
                            levels = c("Down_Up", "Up_Up", "Down_Down", "Up_Down", "all"))) %>%
    arrange(cluster)
...
imap_dfr(PPI_cluster.ego, ~get_stats_graph(.x) %>% 
             as.data.frame %>% mutate(cluster = .y))%>% 
    mutate(cluster = factor(cluster, 
                            levels = c("Down_Up", "Up_Up", "Down_Down", "Up_Down", "all"))) %>%
    arrange(cluster)

imap_dfr(PPI_cluster.ego, ~{vertex_attr(.x) %>% 
        as.data.frame() %>% mutate(cluster = .y) %>%
        mutate(type = ifelse(name %in% (do.call("rbind", prot.cluster) %>% 
                                            filter(cluster == .y)  %>% 
                                            pull(molecule)), "prot", "ego"))}) %>%
...
We also include the TF (protein) -> gene relationship from TF2DNA and Trrust.

(Again, this R chunk is not evaluated because of the size of the database files).
```{r tfome, eval=FALSE}
...
link_gene_prot = rbind(TFome.filtered, coding.filtered) 
...
save(TFome.filtered, file = "./data/TFome_filtered.rda")
save(GRN_PPI, GRN_PPI.ego, PPI_cluster.ego, PPI_cluster, file = "./result/GRN_PPI.rda")
```

```{r tfome_1-2}
load("./data/TFome_filtered.rda")
load("./result/GRN_PPI.rda")
```

We generate some network statistics.
...
We applied a GO terms over representation analysis for each layer.
```{r ora1}
load("./result/luster_info.rda")
load("./result/GRN_PPI.rda")
...
We combined the GO terms pvalue to identified shared terms between omic layers.
```{r ora2}
...
#openxlsx::write.xlsx(x = enrich.df.write, file = "./result/table_hela_ora.xlsx")
```

We used the GO -> Gene/Prot relationship to add this GO term layer to the networks.
```{r go_1}
go.info <- map_dfr(enrich.df$term_id %>% unique, ~get_go_info(.x))

enrich.df <- enrich.df %>% left_join(go.info, by = c("term_id"="name"))
save(enrich.df, file = "./result/enrich_all_filtered.rda")
```
```{r go_2}
load("./result/enrich_all_filtered.rda")
...
write_csv(tmp.unique.cluster, path = "./result/cs1_RWR_unique.csv")
```
...
save(goterm_map, file = "./result/goterm_map.rda")
...
save(unknown_seeds, unknown_seeds_nodes, file ="./result/unknown_seeds.rda")
```

## Metabolite

Finaly, we included a metabolite layer from Kegg [@kegg] with Gene <-> KO and KO <-> GO interactions.

```{r metabolite_1,eval=FALSE}
load("./data/reaction_mapformula_df.rda")
...
```{r metabolite_2, eval=FALSE}

################
# Add metabo layer
################

load(file = ".//result/GRN_PPI_GO.rda")
...
save(FULL_graph, file = "./result/graph_w_metabo.rda")
...
and we cleaned the graph (remove duplicate links, correct layer names, ...)
...
save(FINAL, FINAL_GO_ANNOT, FINAL_GO_ANNOT_short, file = './final/FINAL.rda')
...
## RWR for Multi-omics mechanisms

```{r rw_1}
load("./result/FINAL.rda")
load("./result/unknown_seeds.rda")
load("./result/enrich_all_filtered.rda")
...
#     mc.cores = 4)
# Sys.time()
load("./result/RWR_res_mechanisms.rda")
```
...
RWR_go_mechanisms_annot <- lapply(names(RWR_go_mechanisms), function(x){
    imap_dfr(RWR_go_mechanisms[[x]], ~{ mechanisms_annot(.x, .y)}) %>%
        mutate(CLUSTER = x)}) %>%
    do.call(what = "rbind")
write.csv(RWR_go_mechanisms_annot, file = "./result/RWR_meca.csv")
```
...
# 2. heterogeity: split
load(file = "./result/goterm_map.rda")
...
## explore result
load("./result/RWR_find_annotations.rda")
...
# write_csv(tmp.melt, path = "./result/RWR_annotations.csv")
```

## RWR for intersection between clusters
...
write_csv(x = res_between_intersection_with_go, path = "./result/RWR_cluster_intersection.csv")
